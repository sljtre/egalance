...
export class AddFriendComponent implements OnInit {
  ...
  fillNotFriend = async (n, start) => {
    ...
    for(const line of users) {
      if (line.username.toUpperCase().includes(this.filter.toUpperCase())) {
        this.filteredNumber++;
      }
      ...
      for (let k = 0; k < friends.length; k++) {
        if (friends[k] === line.username) {
          friend = true;
          k = friends.length;
        }
      }
      for (let k = 0; k < demandsSent.length; k++) {
        if (demandsSent[k] === line.username) {
          sent = true;
          k = demandsSent.length;
        }
      }
      for (let k = 0; k < demandsReceived.length; k++) {
        if (demandsReceived[k] === line.username) {
          received = true;
          k = demandsReceived.length;
        }
      }
      if (!friend) {
        this.notFriends.push({user: line, demandSent: sent, demandReceived: received});
      }
    }
